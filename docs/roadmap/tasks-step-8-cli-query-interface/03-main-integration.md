...
- [x] In `crates/chronos-daemon/src/main.rs`, update the `main` function to parse the `Cli` arguments and route to the corresponding handlers (using a `match` on `cli.command`).
- [x] Create `async fn handle_start() -> anyhow::Result<()>`. Move the existing daemon initialization logic (DB, Capture, Vision, Engine, and `run_pipeline`) into this function.
- [x] Create stub handlers for `handle_pause()` and `handle_resume()` that print a "Not yet implemented in MVP" message for now.
- [x] Update `main` signature to return `anyhow::Result<()>`.
- [x] Run `cargo clippy -p chronos-daemon -- -D warnings` and `cargo test -p chronos-daemon`.
- [x] Perform a manual check: Run `cargo run -p chronos-daemon -- --help`, `cargo run -p chronos-daemon -- status`.
