...
- [ ] In `crates/chronos-daemon/src/database.rs`:
  - [ ] Define the `Database` struct:
    ...
    ```
  - [ ] Implement `Database::new(database_url: &str) -> Result<Self>`:
    - [ ] Create connection pool using `SqlitePoolOptions`.
    - [ ] Run migrations using `sqlx::migrate!().run(&pool).await`.
  - [ ] Implement `Database::new_in_memory() -> Result<Self>` for testing.
- [ ] Add `sqlx` with `sqlite` and `runtime-tokio` features to `chronos-daemon/Cargo.toml`.
- [ ] Add `chronos-core` as a dependency to access `Result` and `ChronosError`.
