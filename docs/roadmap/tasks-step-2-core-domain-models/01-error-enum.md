...
- [ ] Create file `crates/chronos-core/src/error.rs`.
- [ ] Write `#[cfg(test)]` block with tests to verify error formatting and `From` conversions *before* implementing the logic.
- [ ] Define the `ChronosError` enum with `Capture`, `Inference`, `Database`, `Config`, and `Timeout` variants.
- [ ] Implement `From<sqlx::Error>` and `From<reqwest::Error>` for automatic error conversion.
- [ ] Define `pub type Result<T> = std::result::Result<T, ChronosError>;`.
- [ ] Run `cargo test -p chronos-core` to verify.
