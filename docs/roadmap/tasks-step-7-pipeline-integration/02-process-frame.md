...
- [x] Implement `pub async fn process_frame(&self, frame: &Frame) -> Result<SemanticLog>` on `CaptureEngine` (using borrowed reference for memory efficiency).
- [x] Call `self.vision.analyze_frame(&frame).await`.
- [x] If successful, call `self.database.insert_semantic_log(&log).await`.
- [x] Return the stored log.
- [x] Write a test `test_process_frame_with_mocks` using `MockVision` and an in-memory DB.
- [x] Write a test `test_process_frame_stores_correct_source_frame_id`.
- [x] Write a test `test_pipeline_handles_vision_error_gracefully` (now refined to verify continuation after error).
- [x] Run `cargo test -p chronos-daemon`.
