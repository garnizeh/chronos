...
- **Configurable Prompt Strategies:** We introduced a `PromptStrategy` enum (`Simple`, `Standard`, `Detailed`). This allows the system to switch between "fast/concise" and "slow/detailed" analysis without changing a single line of inference logic—just a configuration toggle.
...
In `chronos-capture` (specifically in `x11.rs`), when we fallback to the first monitor:
```rust
...
```
> [!NOTE]
> The above snippet uses the `if let` pattern with a warning log to ensure the user knows which monitor is being used when the "primary" flag is missing.
